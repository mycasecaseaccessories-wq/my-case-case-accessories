...
class TelegramApi:
    ...
    async def call(self, method: str, **payload: Any) -> dict[str, Any]:
        async with httpx.AsyncClient(timeout=self.settings.request_timeout) as client:
            response = await client.post(f"{self.settings.telegram_api_url}/{method}", json=payload)
...
async def dispatch_update(bot: TelegramCommerceBot, telegram_api: TelegramApi, update: dict[str, Any]) -> None:
