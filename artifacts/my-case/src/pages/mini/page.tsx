...
const API = ((import.meta.env.VITE_API_BASE_URL as string) || "/api/v1").replace(/\/$/, "");
...
export default function MiniAppPage() {
  ...
  function applyCentralCart(payload: {
  ...
  async function add(product: Product) {
    ...
    const quantity = clampQuantity((existing?.quantity ?? 0) + 1);
    if (sessionState === "verified") {
      const response = await fetch(`${API}/telegram/cart/items`, {
        method: "POST",
        headers: { "Content-Type": "application/json", ...telegramHeaders() },
        body: JSON.stringify({ product_id: product.id, quantity }),
      });
      if (response.ok) applyCentralCart(await response.json());
      else setCartError("Cart ကို server နှင့် sync မလုပ်နိုင်ပါ။");
    } else {
      setCart((current) =>
        existing
          ? current.map((line) =>
  ...
  async function change(productId: string, delta: number) {
    ...
    if (sessionState === "verified") {
      const response =
        nextQuantity <= 0
          ? await fetch(`${API}/telegram/cart/items/${productId}`, {
              method: "DELETE",
              headers: telegramHeaders(),
            })
          : await fetch(`${API}/telegram/cart/items`, {
              method: "POST",
              headers: { "Content-Type": "application/json", ...telegramHeaders() },
              body: JSON.stringify({
    ...
    setCart((current) =>
      current.flatMap((line) =>
        line.product.id === productId
          ? nextQuantity > 0 && nextQuantity <= 999
            ? [{ ...line, quantity: clampQuantity(nextQuantity) }]
            : nextQuantity <= 0
              ? []
              : [line]
          : [line],
  ...
  async function clearCart() {
    if (sessionState === "verified") {
      const responses = await Promise.all(
        cart.map((line) =>
          fetch(`${API}/telegram/cart/items/${line.product.id}`, {
            method: "DELETE",
            headers: telegramHeaders(),
          }),
        ),
      ...
      if (responses.every((response) => response.ok)) {
        const canonical = await fetch(`${API}/telegram/cart`, {
          headers: telegramHeaders(),
        });
        if (canonical.ok) applyCentralCart(await canonical.json());
        else setCartError("Cart ကို server နှင့် sync မလုပ်နိုင်ပါ။");
      } else if (responses.length) {
        setCartError("Cart ကို server နှင့် sync မလုပ်နိုင်ပါ။");
      }
      return;
    }
    setCart([]);
  ...
  async function checkout() {
    ...
    try {
      ...
      const customer =
        customerResponse.ok ? await customerResponse.json() : null;
      if (!customer && customerResponse.status !== 409) throw new Error("customer");
          ...
          <section className="cart-panel" aria-label="Cart">
            ...
            {!cart.length ? (
              ...
              <>
                ...
                <button className="secondary" onClick={clearCart} type="button">
                  Clear cart
                </button>
                <div className="form">
                  <input
